...
if model_name == "01ai/Yi-34B-Chat-4bits":
    extra_body = {"repetition_penalty": 1.1, "stop_token_ids": [7]}
else:
    extra_body = {
...
class UnifiedModelClient:
    ...
    def chat_completions_create(self, messages, model=None, timeout=240, **kwargs):
        ...
        if self.provider == "openai":
            # OpenAI API调用
            response = self.client.chat.completions.create(model=model, messages=messages, timeout=timeout, **kwargs)
            # 转换为统一格式
            return {
                "choices": [
                    {
                ...
                if DASHSCOPE_AVAILABLE:
                    ...
                    if response.status_code == 200:
                        return {
                            "choices": [
                                {
                                    "message": {"content": response.output.choices[0].message.content},
...
def get_model_client(**kwargs):
    ...
    if provider == "openai":
        ...
        return UnifiedModelClient(
            provider="aliyun", model_name=client_model_name, api_key=kwargs.get("api_key", aliyun_api_key)
        )
    else:
    ...
    for retry in range(5):
        try:
            response = client.chat_completions_create(
                messages=messages, timeout=240, **extra_body if extra_body else {}
            )
            ...
            logger.info(
                f"整体风格{understanding.get('style', '')}，适合{understanding.get('target_audience', '')}观看。"
            )
...
```"""
    ...
    for retry in range(5):
        try:
            ...
            response = client.chat_completions_create(messages=messages, timeout=240, **kwargs)
...
def valid_translation(text, translation):
    ...
    for word in forbidden:
        if word in translation:
            return (
                False,
                f"Don't include `{word}` in the translation. Only translate the following sentence and give me the result.",
            )
                ...
                if not success:
                    ...
                    if retry < 4:
                        ...
                        if (
                            ...
                            or (extracted.startswith('"') and extracted.endswith('"'))
                            or (extracted.startswith("'") and extracted.endswith("'"))
                        ):
                        ...
                        if any("\u4e00" <= char <= "\u9fff" for char in extracted):
                            ...
                            logger.warning(
                                f"翻译验证失败，但已从API返回中提取翻译: {original_text[:30]}... -> {extracted[:30]}..."
                            )
                            break
                        else:
                            raise Exception("Invalid translation")
...
def cultural_adaptation(translations, transcript, target_language="简体中文", understanding=None):
    ...
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    logger.info(
        f"{timestamp} 进入第3轮->文化调整： 针对意译的结果，根据目标语言的文化背景和表达习惯，对翻译文本进行适当调整。\n"
    )
...
def reflection_and_refinement(translations, transcript, target_language="简体中文", understanding=None):
    ...
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    logger.info(
        f"{timestamp} 进入第4轮->反思调整： AI对翻译结果自动评估，检测并修正文化语义偏差、流畅度问题及风格一致性等方面的问题。\n"
    )
...
def subtitle_polish(translations, transcript, target_language="简体中文"):
    ...
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    logger.info(
        f"{timestamp} 进入第5轮->字幕精校： 最后对翻译好的字幕进行全面检查，确保字幕与视频同步准确无误，语言表述精准，格式规范统一。\n"
    )
