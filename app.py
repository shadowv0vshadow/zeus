...
default_voice_value = (
    f"{DEFAULT_VOICES.get('zh', 'longyue_v2')} - {zh_voices.get(DEFAULT_VOICES.get('zh', 'longyue_v2'), '')}"
)
...
with gr.Blocks(title="YouDub - 全自动") as do_everything_interface:
    ...
    resolution = gr.Radio(
        ["4320p", "2160p", "1440p", "1080p", "720p", "480p", "360p", "240p", "144p"], label="Resolution", value="1080p"
    )
    ...
    translation_model_provider = gr.Radio(
        ["openai", "aliyun"],
        label="Translation Model Provider",
        value="openai",
        info="选择翻译模型提供商：OpenAI 或 阿里云通义大模型",
    )
    tts_model_radio = gr.Radio(
        ["bytedance", "aliyun", "xtts_v1", "xtts_v2", "xtts_v2.0.2"],
        label="TTS Model",
        value="xtts_v2",
        info="选择语音合成模型：字节跳动、阿里云或 XTTS",
    )
    aliyun_voice_dropdown = gr.Dropdown(
    ...
    tts_model_radio.change(fn=update_voice_dropdown, inputs=[tts_model_radio], outputs=[aliyun_voice_dropdown])
...
def refresh_projects_for_translation(folder):
    ...
    for root, dirs, files in os.walk(folder):
        if "transcript.json" in files:
            ...
            projects.append({"name": project_name, "path": root, "title": title, "has_translation": has_translation})
...
def load_translation_from_project(project_choice, project_paths_dict):
    ...
    return (
        gr.Dataframe(value=table_data, headers=["编号", "原文", "翻译", "使用原声"], interactive=True),
        project_path,
        project_info,
    )
...
def save_translation_edits(project_path, segments_df):
    ...
    logger.debug(
        f"segments_df类型: {type(segments_df)}, 长度: {len(segments_df) if segments_df else 0}, 内容示例: {segments_df[:2] if segments_df and len(segments_df) > 2 else segments_df}"
    )
        ...
        for row_idx, row in enumerate(segments_df):
            ...
            if len(row) >= 4:
                ...
                if segment_num < len(transcript):
                    ...
                    logger.debug(
                        f'片段{segment_num}: translation="{edited_translation[:30]}...", use_original={use_original}'
                    )
                else:
...
with gr.Blocks(title="YouDub - 字幕翻译") as translation_interface:
    ...
    translation_project_dropdown = gr.Dropdown(
        label="选择项目", choices=[], value=None, info="请先选择要编辑翻译的项目（需要包含transcript.json）"
    )
...
def generate_tts_and_load_segments(folder, tts_model, aliyun_voice_str):
    ...
    return (
        tts_result,
        gr.Dropdown(choices=project_choices, value=project_choices[0] if project_choices else None),
        project_paths,
    )
...
def load_project_segments(project_choice, project_paths_dict):
    ...
    return (
        project_info,
        gr.Dataframe(
            value=table_data, headers=["编号", "翻译文本", "有AI配音", "有原声", "使用原声"], interactive=True
        ),
        project_path,
    )
...
def save_config_and_regenerate_audio_wrapper(project_path, segments_df):
    ...
    if segments_df and len(segments_df) > 0:
        ...
        for row_idx, row in enumerate(segments_df):
            ...
            if len(row) >= 3:
                ...
                config[segment_id] = use_original
                debug_info.append(
                    f"  行{row_idx}: {original_segment_id}->{segment_id}, use_original_raw={repr(use_original_raw)} ({type(use_original_raw).__name__}), use_original={use_original}"
                )
    ...
    config_debug = f"\n配置详情:\n" + "\n".join(
        [f"  片段 {k}: {'原声' if v else 'AI配音'}" for k, v in sorted(config.items())]
    )
    if debug_info:
        config_debug += "\n\n解析详情:\n" + "\n".join(debug_info)
...
with gr.Blocks(title="YouDub - 语音合成") as tts_interafce:
    ...
    tts_interface_tts_model = gr.Radio(
        ["bytedance", "aliyun", "xtts_v1", "xtts_v2", "xtts_v2.0.2"],
        label="TTS Model",
        value="xtts_v2",
        info="选择语音合成模型：字节跳动、阿里云或 XTTS",
    )
    tts_interface_voice_dropdown = gr.Dropdown(
        choices=voice_options,
        label="阿里云音色",
        value=default_voice_value,
        visible=False,
    ...
    tts_project_dropdown = gr.Dropdown(
        label="选择项目", choices=[], value=None, info="请先选择要配置的项目（需要包含translation.json）"
    )
    ...
    # 当folder改变时自动刷新项目列表
    tts_folder.change(
        fn=refresh_projects_for_tts, inputs=[tts_folder], outputs=[tts_project_dropdown, tts_project_paths_dict_state]
    )
    ...
    tts_interface_tts_model.change(
        fn=update_voice_dropdown, inputs=[tts_interface_tts_model], outputs=[tts_interface_voice_dropdown]
    )
...
syntehsize_video_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Radio(
            ["4320p", "2160p", "1440p", "1080p", "720p", "480p", "360p", "240p", "144p"], label="分辨率", value="1080p"
        ),
    ],
    outputs="text",
    flagging_mode="never",
